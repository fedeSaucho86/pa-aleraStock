...
from class_migration.modelo import conexion  
from class_migration.modelo import crear_tabla
from class_migration.modelo import alta
from class_migration.modelo import consultar
from class_migration.modelo import baja
from class_migration.modelo import borrar_sql
from class_migration.modelo import modificar
...
def actualizar_treeview(mitreview, product = ""):
...
def modificar_producto(productid, stock, costo, precio, tree, lista_entry):
    clear_text(lista_entry)
    msg, frase = modificar(productid, stock, costo, precio)    
    if msg and frase:
        message(msg, frase)      
    else:
...
def alta_producto(productid, stock, costo, precio, tree, lista_entry):
    clear_text(lista_entry)
    msg, frase = alta(productid, stock, costo, precio)    
    if msg and frase:
        message(msg, frase)      
    else:
        actualizar_treeview(tree)
        
def baja_producto(productid, tree, lista_entry):
    clear_text(lista_entry)
    msg, frase = baja(productid)
    if msg == "Error":
        message(msg, frase)

    if msg == "Delete Item":
        message = ask(msg, frase)
        if message == 'yes':
            borrar_sql(productid)
            actualizar_treeview(tree)
        
        
def consultar_producto(productid, tree, lista_entry):
    clear_text(lista_entry)
    consulta = consultar(productid)
    if consulta == "IS":
        actualizar_treeview(tree, productid)
    else:
        actualizar_treeview(tree)
...
def vista_principal(root):
    ...
    bot_buscar = ttk.Button(root, text='Buscar Producto', command=lambda:consultar_producto(
                                                                                e_producto_id.get(),
                                                                                tree,
                                                                                lista_entry))

    bot_agregar = ttk.Button(root, text='Agregar Producto', command=lambda:alta_producto(
                                                                                e_producto_id.get(),
                                                                                e_stock.get(), 
                                                                                e_precio_costo.get(),
                                                                                e_precio_venta.get(),
    ...
    bot_modificar = ttk.Button(root, text='Modificar Producto', command=lambda:modificar_producto(
                                                                                e_producto_id.get(),
                                                                                e_stock.get(), 
                                                                                e_precio_costo.get(),
                                                                                e_precio_venta.get(),
    ...
    bot_eliminar = ttk.Button(root, text='Eliminar Producto', command=lambda:baja_producto(
                                                                                e_producto_id.get(),
                                                                                tree,
                                                                                lista_entry))
